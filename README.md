...
在上游分层记忆与自动整理之上，增加了渐进召回、空闲时的记忆反思（以及对反思再归纳的二阶总结）、人物/事物相关的轻量实体索引，以及按「最近是否被想起」跟踪记忆访问。常驻档案会在证据充足时逐步更新，让关系和长期认识随真实互动自然修正；情绪侧用连续的愉悦度–唤醒度空间影响「此刻更贴近哪类记忆」，而不是只靠离散标签。
