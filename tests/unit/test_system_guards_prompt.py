...
def test_with_desktop_pet_context_splits_identity_anchor_and_guards_tail() -> None:
    guards = """## 身份与人称
- 我是夜乃桜。

## 勿复读设定
- 勿复述战力"""
    text = with_desktop_pet_context("人格正文", system_guards=guards)
    assert text.index("【身份锚】") < text.index("【人格设定】")
    assert text.index("【人格设定】") < text.index("【互动方式】")
    assert text.index("【互动方式】") < text.index("【演出约束】")
    assert "我是夜乃桜" in text[: text.index("【人格设定】")]
...
def test_load_system_prompt_includes_guards(tmp_path: Path) -> None:
    ...
    assert prompt.index("【身份锚】") < prompt.index("【人格设定】")
    assert prompt.index("【互动方式】") < prompt.index("【演出约束】")
