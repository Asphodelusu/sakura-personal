...
class TauriSettingsResult:
    ...
    # 渐进记忆检索开关；默认关闭，前端未提交时保持 False
    progressive_memory: bool = False
    ...
    *,
    ...
    return TauriSettingsResult(
        ...
        progressive_memory=_optional_bool(
            memory.get("progressive_memory"), default=False
        ),
...
class TauriSettingsProcess(QObject):
        ...
        *lines, self._stdout_buffer = self._stdout_buffer.split("\n")
        ...
        for line in lines:
            ...
            if isinstance(data, dict) and data.get("keep_open"):
                ok = self._persist_settings_result(result, keep_open=True)
                if ok:
                    self.applied.emit(result)
                else:
                    self.failed.emit("设置应用失败，请检查日志或配置文件权限。")
                continue
            ok = self._persist_settings_result(result, keep_open=False)
            self._done = True
            if ok:
                self.completed.emit(result)
            else:
                self.failed.emit("设置保存失败，请检查日志或配置文件权限。")
    ...
    def _persist_settings_result(self, result: object, *, keep_open: bool) -> bool:
        handler = self._persist_handler
        if not callable(handler):
            return True
        try:
            return bool(handler(result, final=not keep_open))
        except Exception as exc:  # noqa: BLE001
            debug_log(
                "TauriSettings",
                "设置持久化回调失败",
                {"error": str(exc), "keep_open": keep_open},
            )
            return False

    def _on_apply_requested(self, request_id: str, result: object) -> None:
        """RPC 路径的「应用」：同步持久化后回复 Rust 侧。"""
        ok = self._persist_settings_result(result, keep_open=True)
        if ok:
            self.applied.emit(result)
        self.resolve_apply_request(request_id, ok=ok)
...
def _debug_log_from_mapping(mapping: dict[str, Any]) -> DebugLogSettings:
    ...
    profile = mapping.get("profile", "info") or "info"
